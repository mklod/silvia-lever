...
class SafetyManager(QObject):
    ...
    def _safety_check(self):
        ...
        time_gap = current_time - self.last_safety_check_time
    
        # If time gap is too large, system likely woke from sleep
        if time_gap > 60:  # More than 1 minute gap
            self.last_data_time = current_time  # Reset timeout
            self.logger.log_warning("System wake-up detected, resetting communication timeout")
