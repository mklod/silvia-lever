...
- **Fix 1 — double-tap to stop.** Single tap on the brew area now only STARTS (when ready); stopping a running brew requires a deliberate quick **double-tap** (`onDoubleClicked`). A single ghost tap can no longer end a brew. Added an on-screen hint under the timer: "tap to start" / "double-tap to stop".
