...
## Current milestone
**Beta — daily-driver.** `redesign-precision` was **merged to `master` 2026-07-06**; all work now lands on `master`. Live UI = "Precision Instrument" (`main_precision.qml` + `precision/` lib). In and working: Boiler Stage 9 (staged dual-heat, arbitration, OPV tuned), functional **shot replay**, **Settings** (dose/scale-cal/autotune), 10 Hz scale pipeline, pump quiet cap (50%) + pot-zero deadband. Pump = FG304: MG304 4 mm gear head + Fluid-o-Tech/Crouzet 24 V BLDC, 0–5 V speed control, TACHO out available (32 pulses/rev, unwired). Thermoblock PID: 46.94/0.516/3155.89 (TL).
**Pending:** hardware test of functional replay (pull a replayed shot); brew-temp stability/accuracy experiment (HEATING.md §8, the open "!!!"); resonance damping hunt (~650–1000 Hz, user); level probe (Silvia Pro) for auto-fill/dry-fire; optional tacho wiring → volumetric metering.

## Session 2026-08-18 (21:29) — Pump noise diagnosed (gear mesh + resonance), quiet cap, pot-zero fix
- **Pump noise root-caused with data, motor swap ruled out.** FFT/order analysis of a recorded pot sweep (100→66→33→0%): gear-mesh orders (7–16× shaft) dominate by 6–11 dB over BLDC commutation (24×) at every speed. A **structural resonance at ~650–1000 Hz** amplifies whichever order lands in it (same 982 Hz peak at different speeds/orders). So an FOC-servo/motor swap ≈ 1 dB — not worth touching the sole pump. Measured true max ~4200 rpm (not 5000).
- **PUMP_PWM_QUIET**: new cap for the open-loop full-speed states (FLUSH + both PRIME fills); 65% killed the scream + OPV chirp, then dropped to **50% (127)** to put fill/flush mesh below the resonance band. User: "dull hum, acceptable." Next lever if wanted: tap-test the bracket/panels/lines for the 650–1000 Hz ring and damp it (butyl/mass) — user investigating.
- **Manual-mode pot-zero bug fixed**: pump kept running at 0% — (a) EXTRACT pwm = pot + handoverOffset so residual takeover offset kept pumping; (b) small pot/ADC offset put 0.2–0.3 V on SPEED_IN where the driver runs its 300 rpm floor. `POT_ZERO_DEADBAND` (10 counts) forces PWM 0 at/below, all three manual-drive sites. Flashed.
- **Pump identity + specs nailed down** (memory + docs): FG304 = MG304 4 mm head (0.3 ml/rev) + Fluid-o-Tech/Crouzet 24 V BLDC w/ integrated 6-step driver. Manual: 0.3–5 V → 300–5000 rpm; **yellow wire = TACHO OUT, 32 pulses/rev (~9.4 µl/pulse)** → future volumetric metering (needs 5 V→3.3 V divider to a free pin, e.g. D2; declined for now). 4 mm gear set confirmed correct (13 bar headroom, finest low-flow control); 9 mm only viable with a stronger motor and would halve rpm.
- Earlier this session (2026-07-22): scale-latency fully fixed — firmware rolling trimmed-mean @80 SPS, UI Repeater churn removed, ChartCard glow via layered strokes + threaded raster (shadowBlur was janking the GUI thread to ~1 Hz during brews only).
...
- **Full UI redesign implemented and switched live** (branch `redesign-precision`). New root `main_precision.qml` + `precision/` component library (Theme singleton, Overline, PButton, StatusBar, ColumnMeter, ChartCard, StatItem, InstrumentCol, Sparkline, PhaseChart, BackChip), reusing the existing CoffeeController backend. True-black monochrome + single red accent; Archivo + JetBrains Mono bundled (`ui/source/fonts/`, FontLoader). Glow via Canvas `shadowBlur` (renders software + GPU). Five screens: Home, Set-temp modal, Brew (live white-mass/red-pressure charts), Profile picker (firmware-accurate phase data in `precision/profiledata.js` + PhaseChart + table), Shot history/replay.
- **Wiring:** profile-row tap → `controller.setProfile()`; brand "SILVIA·LEVER" tap → history; new backend `requestShots()` + `shotsChanged` reads `brew_logs/` into the history list; REPLAY animates the stored curves (ChartCard `clipT`). Old `main.qml` kept intact as fallback.
- **Live-flip:** `run_silvia.py` + `main.py` now load `main_precision.qml`. Deployed + rebooted; grim grab of the real panel confirms it renders correctly (HEAT OFF, SET 250°/203°, PRIMED, real pot/pump values).
- **Known follow-ups:** brew_recorder doesn't store the active profile name (history shows "—"); replay is visual-only (no physical re-run); could promote a dedicated history entry affordance; merge `redesign-precision` → master after live use. Old screenshot harness `_render_screens.py` (old UI) still present.

## Session 2026-06-16 (13:48) — Default steam temp → 250 °F + startup setpoint restore
- **Default steam boiler temp is now 250 °F (121.11 °C)** (was 130 °C / 266 °F). Changed in `firmware/config.h` `DEFAULT_STEAM_TEMP`, `settings_manager.py` default, `main.qml` initial `steamTargetTemp`, and the live Pi `settings.json` (steam_temp 130→121.11, other keys preserved). Firmware recompiled clean (not reflashed — see below).
- **New: setpoints are pushed to the firmware on startup.** Added `_restore_temperatures()` (mirrors the existing scale-cal / PID restore) sending `SET_TEMP BREW/STEAM` on connect (singleShot 500 ms). Previously temps were the *only* persisted setting NOT restored to the firmware on boot, so the firmware used its own config.h defaults until the user opened the setpoint popup — a latent divergence (UI showed brew 95 °C but firmware brewed at its 93 °C default). Now the UI's saved setpoints are authoritative on every boot, so this change took effect **without a firmware reflash**. Verified on reboot: log shows `Restored setpoints: BREW 95.0 STEAM 121.11` → `OK:STEAM_TEMP_SET`.
- Routed voice-to-code item (id 4e1b9ef5). Deployed + rebooted; UI validated headless.

## Session 2026-06-10 (22:40) — Brew stop hardening: double-tap + timer-reset bug
- **Incident diagnosed (from Pi logs `silvia_20260610_205506.log`):** a ~2 min manual brew "stopped at 46 s on its own." Root cause was NOT a timeout — the brew screen's giant central `MouseArea` (covers most of the screen) calls `stopBrew()` on **any single tap** while BREWING, with no button/label/confirmation. A stray capacitive ghost tap (hand near the screen during the long brew) ended the shot. Confirmed: a lone UI-originated `CMD_SENT: STOP` at 46 s, no auto-stop in code (`max_brew_time`=300 s; comm watchdog fed by 5 s PING/PONG). **A 2-min brew is safe** — the only cap is the 5-min safety timeout.
- **Fix 1 — stop = single tap on the timer clock.** (Double-tap was tried first but felt unreliable on the touchscreen — reverted.) The big central area is now **start-only and disabled while BREWING**, so a stray tap anywhere can't end a shot. Stopping is a single tap on a focused `brewStopArea` MouseArea over the clock (enabled only while BREWING). The clock glows amber while brewing to signal it's the stop target; hint reads "tap clock to stop" / "tap to start".
- **Fix 2 — brew-timer reset / double-log bug.** `stopBrew()` left `_brew_start_time` set ("keep time displayed"), so logged durations grew cumulatively (the log showed 46 s **then** 54 s for one shot) and every back-arrow re-logged a bogus `BREW_COMPLETE`. Added a `_brewing` flag (True only between `beginBrew`→`stopBrew`); duration is logged only when a brew was actually running. STOP is still always sent to firmware (safe/idempotent). Displayed time still freezes at final (intended).
- Deployed to Pi 1 (md5 match), Python+QML validated headless, rebooted → UI relaunched clean. **Needs a hot re-test** (see checklist below).
