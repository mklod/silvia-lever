...
RESERVOIR
    │
    ▼
  PUMP (Fluid-o-Tech FG300, 24 V BLDC magnet-drive GEAR pump — NOT vibratory)
       24 V = motor supply only. Speed control is a separate 0–5 V analog
       input, generated by pot → Teensy PWM → 1k/470nF RC filter. The RC
       filter node is ≤5 V (never the 24 V rail) — a 10 V cap is sufficient.
       Datasheet: Dropbox/PROJECTS/ESPRESSO/Silvia mod/MOTOR.PUMP/
    │
    ▼
...
The pump's outlet check valve holds line pressure indefinitely when the pump stops, which prevents backflow damage. The consequence is that the line upstream of V2 cannot bleed by stopping the pump alone — it bleeds only when the OPV setpoint is exceeded (thermal expansion) or the check valve is otherwise relieved. Operationally this is fine because V2 handles all the pressure relief that matters during brewing.
